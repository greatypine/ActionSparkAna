...
/**
  * Description:  用户行为分析_E店维度(正式数据)   
  * @author lyy  
  * @date 2018年5月03日
 */
public class UserActionMong_eshop {
...
//		final LongAccumulator rowNum = spark.sparkContext().longAccumulator("rowN");
		...
		String sql = "SELECT exh,jc ,jt, phone,time ,type   "
				...
				+ "and substring(time,0,10) = '2018-04-23'   )     ";
		...
		JavaPairRDD<String, Row> phoneRowRDD = dataset2.javaRDD().flatMapToPair(new PairFlatMapFunction<Row, String, Row>() {
			private static final long serialVersionUID = 4942131982245494211L;
		...
		//增加分区
		phoneRowCacheRDD.repartition(3);
		//<phone,list<row>>  >> 着个分析 
		JavaPairRDD<String, List<Object[]>> userActionPathRdd = phoneRowCacheRDD.groupByKey().mapToPair(new PairFunction<Tuple2<String,Iterable<Row>>, String, List<Object[]>>() {
			private static final long serialVersionUID = 19822454198224541L;
		...
		JavaPairRDD<String, Integer> depthRDD = userActionPathRddCache.flatMapToPair(new PairFlatMapFunction<Tuple2<String,List<Object[]>>, String, Integer>() {
			...
			private static final long serialVersionUID = 54198224541982241L;
		...
		depthRDD.reduceByKey(new Function2<Integer, Integer, Integer>() {
			private static final long serialVersionUID = 198219822452451L;
			...
			private static final long serialVersionUID = 119819821982198L;
			...
			A : for(int i = 0 ; i < list.size()-1 ; i++){
				...
				if(!"".equals(date1Str) && !"".equals(date2Str)  ){
					...
					if(sdf.parse(date2Str).getTime() - sdf.parse(date1Str).getTime() <= 10000 && i != list.size()-2){
						//继续下个循环
						//记录深度
						returnList.add(new Tuple2<String,Integer>("nullEshopId_"+(obj1.length+1)+"_No",1));
						continue A;
					}else{
						...
						//判断是否发生购买行为
						sql= " select toi.eshop_pro_id  AS eshopProId , toi.order_id AS orderId , "
								+ "toi.create_user_id AS createUserId  , "
								+ " tp.eshop_id as eshopId   from   "
								+ " t_order_item toi "
								+ " JOIN t_product tp ON toi.eshop_pro_id = tp.id  "
								+ " JOIN t_customer tc ON toi.create_user_id = tc.id   "
						...
						if(userBuyList != null && userBuyList.size()>0){
							...
							//获取 频道id和e店id,先暂定为第一条item的频道id和E店id,后期根据匹配上的记录再进行修改
							eshopId =  userBuyList.get(0).get("eshopId") == null ? null :userBuyList.get(0).get("eshopId") +""; 
								...
								B1 : for(Map<String,Object> userBuyMap : userBuyList){
									...
									if(eshopProId != null && !"".equals(eshopProId) && !"null".equals(eshopProId) && contentTag != null && !"".equals(contentTag) ){
										...
										if(groupList != null && groupList.size()>0 ){
											...
											buyFlag = true;
											//将频道和E店id进行重新赋值
											eshopId =  userBuyMap.get("eshopId") ==  null ?  null : userBuyMap.get("eshopId")+"" ;
											//跳出循环
											break B1;
										}
									}
							...
							System.out.println(" is eshop ");
								...
								String eshopId_log = row1.getAs("jc").toString().trim();
								
								if(eshopId_log != null && !"".equals(eshopId_log)){
									B4: for(Map<String,Object> userBuyMap : userBuyList){
										...
										if(eshopProId != null && !"null".equals(eshopProId) && !"".equals(eshopProId)){
											...
											sql = "SELECT id  FROM t_product  "
													+ "  WHERE eshop_id = '"+eshopId_log.trim()+"' "
				...
				if(buyFlag){
					returnList.add(new Tuple2<String,Integer>(key+(obj1.length+1)+"_Yes",1));
				}else{
					
					returnList.add(new Tuple2<String,Integer>(key+(obj1.length+1)+"_No",1));
				}
			}//end A
		}
